...
- **Custom `Adw.Dialog` Instantiation**: To create a custom, transient dialog window that is modal, the class must inherit from `Adw.Dialog`. The `transient_for` and `modal` states MUST be set as keyword arguments in the constructor (e.g., `MyDialog(transient_for=parent_window, modal=True)`). The PyGObject bindings map the `transient-for` GObject property to the `transient_for` keyword argument.
    *CRITICAL: Do not attempt to call a `set_transient_for()` method, as it does not exist in the Python bindings and will cause an `AttributeError`. The correct pattern is to pass these properties directly to the constructor.*
