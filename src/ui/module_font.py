...
class FontManager(Adw.Bin):
    ...
    def __init__(self, **kwargs):
        ...
        self.status_page = Adw.StatusPage(
            title="No Fonts", icon_name="font-x-generic-symbolic"
        )
    ...
    def _display_font_preview(self, font):
        ...
        if self.project_manager.project_path and os.path.exists(
            os.path.join(self.project_manager.project_path, font.file_path)
        ):
            font_map.load_font(
                os.path.join(self.project_manager.project_path, font.file_path)
            )
    ...
    def _on_add_font(self, button):
        ...
        dialog.set_title("Add Font")

        font_filter = Gtk.FileFilter()
        font_filter.set_name("Font Files")
        font_filter.add_pattern("*.ttf")
        font_filter.add_pattern("*.otf")
        dialog.set_default_filter(font_filter)

        dialog.open(self.get_root(), None, self._on_add_font_response)

    def _on_add_font_response(self, dialog, result):
        try:
            ...
            new_font = self.project_manager.add_font(
                id=f"font_{len(self.model) + 1}",
                name=font_name,
